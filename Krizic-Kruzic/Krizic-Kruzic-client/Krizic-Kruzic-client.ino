  ...
  if (udp.listen(1234)) {
    ...
    udp.onPacket([](AsyncUDPPacket packet) {
      ...
      PorukaP = String( (char*) packet.data()); // Board: 211021212
        /* Serial.print(", Data: ");
        Serial.write(packet.data(), packet.length());
        Serial.print("PorukaP: ");
        Serial.print(PorukaP); */
      if (PorukaP.startsWith("SBoard:", 0)) {
        //Serial.println("Got an #SBoard:# message from the server");
...
void playGame()
{
  ...
  { Serial.print("moves=");Serial.println(moves);
    if (moves % 2 == 0) //Prvi je Player One (moves % 2 == 0) ili Player TWO ako je (moves % 2 == 1)
    {                   //Pazite da je server postavljen suprotno
      computerMove();             Serial.println("387 computerMove");
      printBoardCM();               Serial.println("389printBoard");
      checkWinner();              Serial.println("390 checkWinner");
    } else
    {
      playerMove();               Serial.println("392 playerMove");
      printBoardPM();               Serial.println("395 printBoard");
      checkWinner();              Serial.println("396 checkWinner");
    }
...
{
  int i = 0;
  Serial.println("CBoard: [");
  String Poruka = "CBoard:";
  for (i = 0; i < 9; i++)
  {
    Serial.print(board[i]);
    Serial.print(",");
  ...
  Serial.print("]");
  Serial.print("C Poruka:");
  Serial.println(Poruka);

  udp.connect(IPAddress(192,168,0,101), 1234); // IP adresa klijenta
  udp.print(Poruka);
}
