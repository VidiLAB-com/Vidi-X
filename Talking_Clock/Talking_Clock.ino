...
// postavite ime i password od vaše Wi-Fi mreže na koju se želite spojiti
const char *ssid     = "Ime wireless mreže";
const char *password = "Password wireless mreže";

// definiramo objekt vremena
WiFiUDP ntpUDP;

// Definirajte 'time server pool' i vremenski pomak u sekundama (kasnije može
// biti promijenjen naredbom 'setTimeOffset()'. Možete definirati i 'update'
// interval u sekundama, te ga promijeniti koristeći 'setUpdateInterval()'.
// 7200 sekundi je 2 sata (2 sata * 60 minuta * 60 sekundi)
const long utcOffsetInSeconds = 7200;
NTPClient timeClient(ntpUDP, "europe.pool.ntp.org", utcOffsetInSeconds);

// brojevi i neke rijeći definiramo kao heksadececimalni niz
...
// Biblioteke za ILI9341 TFT LCD zaslon
...
// Analogni pin 14 spojen je na mikrofon s mikrodfonskim predpojačalom
...
// j je pomoćna varijabla
int j = 0;

// val, valMin i valMax su varijable koje ćemo koristiti za detektiranje jačine zvuka
int val = 0;
int valMin = 4096;     // ove varijeble postavljemo na besmisleno velike vrijednosti
int valMax = -4096;    // ..koje nam softver neće javiti

void setup() {
  // Postavi brzinu serijske komunikacije
  Serial.begin(115200);
  ...
  // pokreni klijent za vrijeme
  timeClient.begin();

  // provjeri koliko je sati
  timeClient.update();
  pinMode(LED_BUILTIN, OUTPUT);   // Postavi STAT diodu kao OUTPUT
  pinMode(analogInPin1, INPUT);   // Postavi pin senzora kao INPUT

  while (!Serial)
    ; //pričekaj serijski port

  // Ispiši kako se zove ova skica
  Serial.println(F("START " __FILE__ " from " __DATE__ "\r\nUsing library version " VERSION_TALKIE));

  Serial.print("Voice queue size is: ");
  Serial.println(Voice.sayQ(spPAUSE1)); // ovo inicijalizira govor i hardver

  Serial.println();
  Serial.println("Telling time!");
  ...
  // Pozovi funkciju koja ispisuje pozdravnu poruku
  startScreen();
  
  //odspoji WiFi jer nam više ne treba
  //a trebaju nam pinovi koji su zauzeti kada
  //je WiFi uključen
  WiFi.mode(WIFI_OFF);
}
...
void startScreen() {
  ...
  TFT.setTextSize(2);
  TFT.println("Whistle or clap your hands");
  TFT.setTextSize(1);
  TFT.println("TO UPDATE TIME");
  TFT.setTextColor(ILI9341_RED);
  TFT.setTextSize(3);
}
...
boolean zvuk() {
  j = -1;
  valMin = 4096;     // ove varijeble postavljemo na besmisleno velike vrijednosti
...
// Funkcija koja provjerava koliko je sati te nam govori rezultat
boolean sat() {
  ...
  Voice.sayQ(spPAUSE1);
  delay(3000);    // Pričekaj 3 sekunda
  startScreen();  // a zatim pokaži pozdravni ekran
}
